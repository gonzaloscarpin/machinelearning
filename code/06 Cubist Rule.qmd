---
title: "06 Cubist rule-based"
format: html
---

# Random Forest Model for Cotton Lint Yield Prediction

This document implements a Cubist-Rule regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(rules)         # Fast implementation of CubistRule
...
We define a cubist_rules model with hyperparameters to be tuned.

```{r model_specification}
# Define the Cubist Rules model with tunable parameters
cubist_spec <-
  cubist_rules(committees = tune(), #A non-negative integer (no greater than 100) for the number of members of the ensemble.
                        neighbors = tune(), #An integer between zero and nine for the number of training set instances that are used to adjust the model-based prediction.
...
cubist_grid_result <- tune_race_anova(
  object = cubist_spec,
...
This analysis demonstrates the application of Cubist-Rule for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.