---
title: "17 Ensembles of Neural Networks"
format: html
---

# Ensembles of neural networks Model for Cotton Lint Yield Prediction

This document implements a bag_MLP regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(baguette)       # Fast implementation of MLP
...
We define a bag_mlp model with hyperparameters to be tuned.

```{r model_specification}
# Define the bagmlp model with tunable parameters
bagmlp_spec <- 
  # Specifying mlp as our model type, asking to tune the hyperparameters
  bag_mlp(hidden_units = tune(),  ## Hidden Units
...
bagmlp_grid_result <- tune_race_anova(
  object = bagmlp_spec,
...
This analysis demonstrates the application of bag_MLP for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.
