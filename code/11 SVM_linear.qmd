---
title: "11 Linear Support Vector Machines"
format: html
---

# Linear Support Vector Machines Model for Cotton Lint Yield Prediction

This document implements a SVM_l regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
We define a svm_l model with hyperparameters to be tuned.

```{r model_specification}
# Define the Linear Support Vector Machines model with tunable parameters
svm_linear_kernlab_spec <-
  svm_linear(cost = tune(), #A positive number for the cost of predicting a sample within or on the wrong side of the margin
...
svm_l_grid_result <- tune_race_anova(
  object = svm_linear_kernlab_spec,
...
This analysis demonstrates the application of SVM_l for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.