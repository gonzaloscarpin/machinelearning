---
title: "13 K-nearest neighbors"
format: html
---

# K-nearest neighbors Model for Cotton Lint Yield Prediction

This document implements a kNN regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
We define a nearest_neighbor model with hyperparameters to be tuned.

```{r model_specification}
# Define the kNN model with tunable parameters
kNN_spec <- 
  # Specifying rf as our model type, asking to tune the hyperparameters
  nearest_neighbor(neighbors = tune(), #A single integer for the number of neighbors to consider 
                   weight_func = tune(), #type of kernel function used to weight distances between samples.
                   dist_power = tune() #A single number for the parameter used in calculating Minkowski distance
                   ) %>% 
  # Specify the engine
  set_engine("kknn") %>% 
  # Specifying mode  
...
kNN_grid_result <- tune_race_anova(
  object = kNN_spec,
...
This analysis demonstrates the application of kNN for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.