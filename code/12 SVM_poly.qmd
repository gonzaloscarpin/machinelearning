---
title: "12 Polynomial Support Vector Machines"
format: html
---

# Polynomial Support Vector Machines Model for Cotton Lint Yield Prediction

This document implements a SVM_p regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(baguette)         # Fast implementation of SVM
...
We define a svm_poly model with hyperparameters to be tuned.

```{r model_specification}
# Define the Poly Support Vector Machines model with tunable parameters
svm_poly_kernlab_spec <-
  svm_poly(cost = tune(),  #A positive number for the cost of predicting a sample within or on the wrong side of the margin
           degree = tune(), #A positive number for polynomial degree.
           scale_factor = tune(), #A positive number for the polynomial scaling factor.
           margin = tune()) %>% #A positive number for the epsilon in the SVM insensitive loss function
  set_engine('kernlab') %>%
  set_mode('regression')
...
svm_p_grid_result <- tune_race_anova(
  object = svm_poly_kernlab_spec,
...
This analysis demonstrates the application of SVM_p for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.