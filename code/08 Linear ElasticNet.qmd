...
library(glmnet)         # Fast implementation of ElasticNet
...
We define a linear_reg model with hyperparameters to be tuned.

```{r hyperparameters}
enet_spec <- 
  # Specifying linear_reg as our model type, asking to tune the hyperparameters
  linear_reg(penalty = tune(), #Amount of Regularizatio
...
enet_grid_result <- tune_grid(object = enet_spec,
                      grid = enet_grid,
...
final_spec <- linear_reg(penalty = best_rmse_2$penalty,
                         mixture = best_rmse_2$mixture
                         ) %>% 
  set_engine("glmnet")
...
This analysis demonstrates the application of Elastic-Net for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.