...
# Random Forest Model for Cotton Lint Yield Prediction

This document implements a RuleFit regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(rules)         # Fast implementation of RuleFit
...
We define a recipe for data preprocessing, removing unnecessary columns and adding step_dummy, zv and normalize. This process followed recomendation for Tidymodels
...
We define a rule_fit model with hyperparameters to be tuned.

```{r model_specification}
# Define the RuleFit model with tunable parameters
rule_fit_xrf_spec <-
  rule_fit(mtry = tune(), #A number for the number (or proportion) of predictors that will be randomly sampled at each split when creating the tree models
           trees = tune(), #An integer for the number of trees contained in the ensemble
           min_n = tune(), #An integer for the minimum number of data points in a node that is required for the node to be split further.
           tree_depth = tune(), #An integer for the minimum number of data points in a node that is required for the node to be split further.
           learn_rate = tune(), #A number for the rate at which the boosting algorithm adapts from iteration-to-iteration
           loss_reduction = tune(), #A number for the reduction in the loss function required to split further
           sample_size = tune(), #A number for the number (or proportion) of data that is exposed to the fitting routine
           penalty = tune() #L1 regularization parameter
           ) %>%
  set_engine('xrf',
             counts = FALSE
             ) %>%
...
rule_grid_result <- tune_race_anova(
  object = rule_fit_xrf_spec,
...
This analysis demonstrates the application of Rule Fit for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.