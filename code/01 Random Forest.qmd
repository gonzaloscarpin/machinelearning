---
title: "01 Random Forest"
format: html
---

# Random Forest Model for Cotton Lint Yield Prediction

This document implements a Random Forest regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
rf_spec <- 
  # Specifying Random Forest with parameters to tune
  rand_forest(
  ...
  # Use ranger as the computation engine
  set_engine("ranger") %>% 
  # Specify we're doing regression
  set_mode("regression")
...
rf_grid_result <- tune_race_anova(
  object = rf_spec,
...
compare_hyperparameters <- function(params_df, recipe, split) {
  ...
  for(i in 1:nrow(params_df)) {
    ...
    # Create model specification with current parameters
    set.seed(10)
...
final_spec <- rand_forest(
  ...
  # Use ranger with permutation importance for later interpretation
  set_engine("ranger", importance = "permutation") %>% 
  # Regression mode
  set_mode("regression")
...
This analysis demonstrates the application of Random Forest for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.