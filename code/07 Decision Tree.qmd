---
title: "07 Decision trees"
format: html
---

# Decision trees Model for Cotton Lint Yield Prediction

This document implements a Decision tree regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(partykit)
library(bonsai)         # Fast implementation of decision tree
...
We define a decision tree model with hyperparameters to be tuned.

```{r model_specification}
# Define the CART model with tunable parameters
cart_spec <- 
  decision_tree(tree_depth = tune(), #maximum depth of the tree
...
cart_grid_result <- tune_race_anova(object = cart_spec,
...
This analysis demonstrates the application of Decision Tree for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.