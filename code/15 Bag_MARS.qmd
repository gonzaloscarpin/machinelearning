---
title: "15 Ensembles of MARS models"
format: html
---

# Ensembles of MARS models for Cotton Lint Yield Prediction

This document implements a MARS regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(baguette)       # Fast implementation of bag_MARS
...
We define a bag_MARS model with hyperparameters to be tuned.

```{r model_specification}
# Define the MARS model with tunable parameters
bag_mars_spec <-
  bag_mars(
    num_terms = tune(), #number of features that will be retained in the final model
    prod_degree = tune(), #the highest possible interaction degree
    prune_method = tune() #pruning method
  ) %>% 
  # Specify the engine
  set_engine("earth") %>% 
  # Specifying mode  
...
bag_mars_grid_result <- tune_race_anova(
  object = bag_mars_spec,
...
bag_mars_grid_result$.metrics[[1]]
```
...
best_rmse <- bag_mars_grid_result %>% 
...
This analysis demonstrates the application of bag_MARS for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.