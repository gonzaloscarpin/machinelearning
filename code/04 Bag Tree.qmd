---
title: "04 Ensembles of decision trees"
format: html
---

# Ensembles of decision trees Model for Cotton Lint Yield Prediction

This document implements a BagTree regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(baguette)         # Fast implementation of BagTree
...
We define a bag_tree model with hyperparameters to be tuned.

```{r model_specification}
# Define the BagTree model with tunable parameters
bag_spec <- 
  bag_tree(tree_depth = tune(), #An integer for the maximum depth of the tree
           min_n = tune(), #An integer for the minimum number of data points in a node that is required for the node to be split further.
           cost_complexity = tune() #A positive number for the the cost/complexity parameter
  ) %>% 
  # Specify the engine
  set_engine("rpart") %>% 
  # Specifying mode  
...
bag_grid_result <- tune_race_anova(
  object = bag_spec,
