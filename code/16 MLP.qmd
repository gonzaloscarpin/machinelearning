---
title: "16 Multilayer Perceptron"
format: html
---

# Multilayer Perceptron Model for Cotton Lint Yield Prediction

This document implements a MLP regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
We define a mlp model with hyperparameters to be tuned.

```{r model_specification}
# Define the mlp model with tunable parameters
mlp_spec <- 
  # Specifying mlp as our model type, asking to tune the hyperparameters
  mlp(hidden_units = tune(),  ## Hidden Units
...
mlp_grid_result <- tune_race_anova(
  object = mlp_spec,
...
This analysis demonstrates the application of MLP for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.