---
title: "10 Partial least squares"
format: html
---

# Partial Least Squares Model for Cotton Lint Yield Prediction

This document implements a PLS regression model to predict cotton lint yield based on genotypic, management, and environmental factors. The workflow includes data preparation, model training with hyperparameter tuning, validation, and interpretation using Shapley values.
...
library(plsmod)         # Fast implementation of PLS
...
We define a pls model with hyperparameters to be tuned.

```{r model_specification}
# Define the PLS model with tunable parameters
pls_mixOmics_spec <- pls(num_comp = tune(), ## Components
                         predictor_prop = tune() #Proportion of Predictors
                         ) %>% 
  set_engine("mixOmics") %>% 
  set_mode("regression") %>% 
  translate()
...
pls_grid_result <- tune_race_anova(object = pls_mixOmics_spec,
...
This analysis demonstrates the application of PLS for predicting cotton lint yield. The model achieves good predictive performance, and Shapley values provide insights into the contribution of different features to the predictions.